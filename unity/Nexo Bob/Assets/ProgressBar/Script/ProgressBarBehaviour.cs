...
namespace ProgressBar
{
	...
	public class ProgressBarBehaviour : MonoBehaviour, IIncrementable, IDecrementable
	{
		...
		public float IncreaseWithValue;
		public Image rewardPic;
		public int maxStars;
		public string rewardForLevel;
		...
		void Update ()
		{
			...
			if (Value < 100) {
				rewardPic.color = Color.black;
			} else if (Value >= 100) {
				rewardPic.color = Color.white;
			}
		}
		...
		void Start ()
		{
			/*IncrementValue (IncreaseWithValue);
			PlayerPrefs.SetFloat ("Level1Stars", 15);
			float increment = (PlayerPrefs.GetFloat ("Level1Stars") / maxStars) * 100;
			Debug.Log (increment);
			IncrementValue (increment);*/
			PlayerPrefs.SetFloat ("ForestStars", 15);
			fillBar ();
		}
